        ...
        <div class="col-md-12">
            <div class="card">
                ...
                <div class="card-header d-flex justify-content-between align-items-center" data-bs-toggle="collapse"
                    ...
                    <h5 class="mb-0">
                        <i class="fas fa-laptop-code me-2"></i>Keterampilan Komputer<span
                            class="required"id="{{ $section_prefix ?? '' }}computerSkillsRequired">*</span>
                    </h5>
...
<script>
    ...
    `;
                ...
                const computerSkillsRequiredAsterisk = document.getElementById(
                `${sectionPrefix}computerSkillsRequired`);

                // Fungsi untuk memeriksa dan menghapus/menambahkan tanda bintang wajib
                function checkAndRemoveComputerSkillsRequiredAsterisk() {
                    let anySkillHasProficiency = false;
                    const skillItems = allComputerSkillsContainer.querySelectorAll('.computer-skill-item');

                    skillItems.forEach(item => {
                        const proficiencyField = item.querySelector('select[name$="[proficiency]"]');
                        const skillNameField = item.querySelector('input[name$="[skill_name]"]');

                        const isPredefinedItem = skillNameField.hasAttribute('readonly');

                        if (isPredefinedItem && proficiencyField.value.trim() !== '') {
                            // Skill yang sudah ada (predefined) diisi tingkat penguasaannya
                            anySkillHasProficiency = true;
                        } else if (!isPredefinedItem && (skillNameField.value.trim() !== '' || proficiencyField
                                .value.trim() !== '')) {
                            // Skill dinamis diisi nama atau tingkat penguasaannya
                            anySkillHasProficiency = true;
                        }
                    });
if (anySkillHasProficiency && computerSkillsRequiredAsterisk) {
                // Jika ada setidaknya satu skill yang diisi, dan tanda bintang ada, hapus tanda bintang
                computerSkillsRequiredAsterisk.remove();
            } else if (!anySkillHasProficiency && !computerSkillsRequiredAsterisk) {
                // Jika tidak ada skill yang diisi, dan tanda bintang tidak ada, tambahkan kembali tanda bintang
                const h5Element = mainCardHeaderComputerSkills.querySelector('h5');
                h5Element.insertAdjacentHTML('beforeend',
                    `<span class="required" id="${sectionPrefix}computerSkillsRequired">*</span>`);
                // Perbarui referensi ke elemen tanda bintang yang baru ditambahkan
                computerSkillsRequiredAsterisk = document.getElementById(`${sectionPrefix}computerSkillsRequired`);
            }
        }
    ...
    `;
                        if (allComputerSkillsContainer) {
                            allComputerSkillsContainer.insertAdjacentHTML('beforeend', skillHtml);
                                    ...
                                    checkAndRemoveComputerSkillsRequiredAsterisk();
        ...
        checkAndRemoveComputerSkillsRequiredAsterisk();
                    ...
                    if (addOtherComputerSkillButton) {
                        addOtherComputerSkillButton.addEventListener('click', function() {
                            ...
                            if (allCurrentItemsValid) {
                                ...
                                const firstInvalidField = allComputerSkillsContainer.querySelector(
                                    '.is-invalid');
                            ...
                            if (isDynamicSkill) { // HANYA HAPUS JIKA isDynamic = true
                                if (confirm('Apakah Anda yakin ingin menghapus keterampilan ini?')) {
                                    ...
                                    reindexComputerSkills(sectionPrefix, allComputerSkillsContainer);
                                                            checkAndRemoveComputerSkillsRequiredAsterisk(); // Periksa kembali setelah menghapus
                    ...
                    function reindexComputerSkills(prefix, container) {
                        ...
                        skillItems.forEach((item, index) => {
                            ...
                            item.querySelectorAll(
                                    'input[name^="computer_skills["], select[name^="computer_skills["]')
                                .forEach(
                                    input => {
                    ...
                    document.querySelectorAll('.save-section-btn').forEach(button => {
                        button.addEventListener('click', async function(e) {
                            ...
                            if (!sectionElement) {
                                console.error(
                                    `Section element not found for ID: ${sectionCollapseId}`);
                                return;
                            }
                            ...
                            if (sectionId === 'computer_skills') {
                                ...
                                skillItems.forEach(item => {
                                    ...
                                    if (isPredefinedItem && proficiencyField.value
                                    .trim() !==
                                        '') {
                                        anySkillFilled = true;
                                    } else if (!isPredefinedItem && (skillNameField.value
                                            .trim() !== '' || proficiencyField.value
                                        .trim() !== ''
                                        )) {
                                        anySkillFilled = true;
                                    }
                                ...
                                if (!anySkillFilled) {
                                    ...
                                    skillItems.forEach((item, idx) => {
                                        ...
                                        const isPredefinedItem = skillNameField
                                            .hasAttribute(
                                        ...
                                        if (isPredefinedItem) { // Untuk skill predefined (nama readonly)
                                            ...
                                            if (skillNameField.value.trim() !== '' ||
                                                ...
                                                if (skillNameField.value.trim() !== '' &&
                                                    ...
                                                    proficiencyField.classList.remove(
                                                        'is-invalid');
                                                    let errorMsgProficiency =
                                                        proficiencyField
                                                        .parentElement.querySelector(
                                                            '.error-message');
                                                    if (errorMsgProficiency)
                                                        errorMsgProficiency
                                                        .style.display = 'none';
                                                }
                                            } else {
                            ...
                            if (isValid) {
                                ...
                                try {
                                    const url =
                                    `{{ url('/dashboard/save-section') }}/${sectionId}`;
                                    ...
                                    if (response.ok) {
                                        ...
                                        sectionElement.querySelectorAll('.is-invalid').forEach(el =>
                                            el
                                            .classList.remove('is-invalid'));
                                        sectionElement.querySelectorAll('.error-message').forEach(
                                            el =>
